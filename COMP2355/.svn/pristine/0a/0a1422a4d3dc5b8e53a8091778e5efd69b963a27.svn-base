...
BitVector::BitVector(int bitsToBeStored) : bitmapSize((bitsToBeStored + 7) / 8), bitmap(new uint8_t[(bitsToBeStored + 7) / 8]) {
...
void BitVector::fill() {
    ...
    for (int i = 0; i < bitmapSize; i++) { // go through all uint8_ts
        bitmap[i] = 0xFF; // set them equal to 1
    }
}
...
void BitVector::clear() {
    // clears all bits
    for (int i = 0; i < bitmapSize; i++) { // go through all uint8_ts
        bitmap[i] = 0x0; // set them equal to 0
    }
}
...
bool BitVector::getBit(int index) {
    ...
    uint8_t temp = bitmap[bitmapElement] >> (7 - bitInElement); // right-shift uint8_t element by 1 less than bit location to LSB
...
void BitVector::setBit(int index, int value) {
    ...
    uint8_t temp = bitmap[bitmapElement];
...
uint8_t BitVector::getBitNumber(int index) { // returns which bit in the byte corresponds to byte number index
    return index % 8;
}
