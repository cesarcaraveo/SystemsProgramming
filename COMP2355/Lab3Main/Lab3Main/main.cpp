...
//  Lab3Main
//
//  Created by Cesar Caraveo on 1/11/18.
...
{
    uint32_t temp = 0; // initializes temp variable used to store final product
    temp = ((input & (1 << first)) << first); // extracts first bit
    
    for (int i = first; i <= (32 / step); i+=step) { // goes through all bits
        temp = temp | (input & (1 << (i + step))); // extracts and adds bit to temp
    }
    
    return temp; // returns full temp variable
}
...
{
    uint32_t temp = 0; // creates temp variable used to store final result
    temp = temp | (input & 1); // extracts first bit
    
    for (int i = 0; i <= (32 / step); i+=step) { // goes through all bits
        temp = temp | ((input & (1 << (i + step))) >> 1); // extracts appropriate bit and adds it to temp
    }
